...
def schedule_sparse_dense_cuda_simple_local_cache(outs):
    ...
    def _callback(op):
        if op.tag == "sparse_dense_bsrmm":
            ...
            data_local = s.cache_read(data, 'local', [y_bsrmm])
...
def schedule_sparse_dense_cuda_allreduce_cache_w_indptr_w_indices(outs):
    ...
    traverse_inline(s, outs[0].op, _callback)
    return s
def schedule_sparse_dense_cuda_allreduce(outs, cfg):
    """Create schedule for sparse dense"""
    s = te.create_schedule([x.op for x in outs])
...
def test_sparse_dense_bsr(M, N, K, BS_R, BS_C, density):
    ...
    task = autotvm.task.create("tutorial/block_sparse",
    ...
    if TUNE:
        tuner.tune(n_trial=500,
                ...
                callbacks=[autotvm.callback.log_to_file('block_sparse.log')])
    ...
    with autotvm.apply_history_best('block_sparse.log'):
...
@autotvm.template("tutorial/block_sparse")
def block_sparse_template(W_sp_np_data_shape, W_sp_np_indices_shape, W_sp_np_indptr_shape, X_np_shape):
    ...
    s = schedule_sparse_dense_cuda_allreduce([Y], cfg)
